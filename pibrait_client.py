...
class PiBrait(object):
    def __init__(
            ...
            localiser_ip=None, 
            localiser_port=8080,
            light_pos = np.array([1,1],dtype=np.float64)
        ):
        self.ip = ip
        self.port = port
    ...
    def sense(self):
        ...
        try:
            ...
            group_number = 0 # TODO: Check if this is needed. This is set to 0 to prevent potential unknown issues with legacy code
            ...
            T_WR = np.array([ # Robot in the world frame
                [ c,-s, x],
                [ s, c, y],
                [ 0, 0, 1]
            ],dtype=np.float64)
            P_Wl = T_WR @ np.array([0.1,-0.1,1],dtype=np.float64).reshape(3,1) # Position bat's left sensor in world frame
            P_Wr = T_WR @ np.array([0.1, 0.1,1],dtype=np.float64).reshape(3,1) # Position bat's right sensor in world frame
            d = np.array([ # Distances between each sensor and the light source.
                np.linalg.norm(P_Wl[:2,0].flatten() - self.light_pos.flatten()),
                np.linalg.norm(P_Wr[:2,0].flatten() - self.light_pos.flatten())
            ])
            sensors = 4.9 / (10 * d**2 + 4.9) # Scary maths that converts our distances into sensor readings.
            return sensors
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='PiBrait client')
    parser.add_argument('--ip', type=str, default='localhost', help='IP address of PiBrait')
    parser.add_argument('--port', type=int, default=8080, help='Port of PiBrait')
    ...
    bot = PiBrait(args.ip, args.port, args.localiser_ip, args.localiser_port)
    img = bot.getImage()
