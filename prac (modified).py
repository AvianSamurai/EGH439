...
if __name__ == "__main__":
    ...
    while(pose[0] > MIN_LIM and pose[1] > MIN_LIM and pose[0] < MAX_LIM and pose[1] < MAX_LIM):
        pose = bot.getLocalizerPose(9) if USE_LOCALIZER else (0, 0, 0);
        [motor_left, motor_right] = DrivetoLine(a,b,c,pose)
        motor_left= round(motor_left)
        motor_right= round(motor_right)
        print(f'motorLeft: {motor_left}, Motor_right:{motor_right}')
        bot.setVelocity(motor_left=motor_left, motor_right=motor_right, duration=None, acceleration_time=None)
